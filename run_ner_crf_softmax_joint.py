...
MODEL_CLASSES = {
    ...
    'bert': (BertConfig, BertCrfSoftmaxForJoint, BertTokenizer),
}
...
def train(args, train_dataset, model, tokenizer):
    ...
    linear_param_optimizer = list(model.ner_classifier.named_parameters())
    linear_visit_param_optimizer = list(model.visit_classifier.named_parameters())
    linear_ride_param_optimizer = list(model.ride_classifier.named_parameters())
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in linear_visit_param_optimizer if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay, 'lr': args.crf_learning_rate},
        {'params': [p for n, p in linear_visit_param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
         'lr': args.crf_learning_rate},

        {'params': [p for n, p in linear_ride_param_optimizer if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay, 'lr': args.crf_learning_rate},
        {'params': [p for n, p in linear_ride_param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
         'lr': args.crf_learning_rate}
    ]
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # visit_labels=None, ride_labels=None
...
def evaluate(args, model, tokenizer, prefix=""):
    # TODO: cal visit_type and ride_type
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        visit_label_list = processor.get_visit_labels()
        ride_label_list = processor.get_ride_labels()
        if data_type == 'train':
        ...
        features = convert_examples_to_features(examples=examples,
                                                ...
                                                visit_label_list=visit_label_list,
                                                ride_label_list=ride_label_list,
...
def main():
    ...
    config.update({"ner_num_labels": ner_num_labels,
                   "visit_num_labels": len(visit_label_list),
                   "ride_num_labels": len(ride_label_list)})
